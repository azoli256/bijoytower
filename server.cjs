...
async function start({port = Number(process.env.PORT || 3000), dataDir = path.join(__dirname, 'data')} = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if(req.method==='GET' && ['/app.js','/style.css','/accounting.js','/i18n.js','/themes/Version_Green.css','/themes/manifest.json'].includes(url.pathname)){
        const type=url.pathname.endsWith('.css')?'text/css; charset=utf-8':url.pathname.endsWith('.json')?'application/json; charset=utf-8':'text/javascript; charset=utf-8';
