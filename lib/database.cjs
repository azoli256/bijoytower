...
function clientView(database, session) {
  ...
  if(session.role !== 'admin') state.flats = state.flats.map(f=>({...f,mobile:f.flatNo===session.flatNo?f.mobile:''}));
  return {revision: database.revision, session: session.role === 'admin' ? {role: 'admin'} : {role: 'resident', flat}, state};
}
