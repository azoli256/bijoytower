...
function clientView(database, session) {
  ...
  if(session.role !== 'admin'){
    ...
    state.flats=state.flats.filter(f=>allowed.has(f.flatNo)).map(f=>({...f,mobile:phoneKey(f.mobile)===key?f.mobile:'',contacts:f.contacts.map(c=>({...c,phone:phoneKey(c.phone)===key?c.phone:''}))}));
    state.entries=state.entries.map(e=>allowed.has(e.flatNo)?e:{...e,flatNo:'',payer:'',note:'',sourceId:'',reference:undefined,receiptNo:undefined,paymentMethod:undefined});
    state.invoices=state.invoices.filter(b=>allowed.has(b.flatNo));
